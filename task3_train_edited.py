...
selected_columns = ['report_date','total_purchase_amt','total_redeem_amt','mfd_daily_yield', 'Interest_O_N']
...
df_new = df_new.withColumn("report_date", F.date_format(F.col('report_date'), 'yyyyMMdd'))
df_new = df_new.withColumn("report_date", F.col("report_date").cast("long"))
df_new.write.csv(f"d:/金融大数据处理技术作业/lab4/output/test{current_time}", header=True)


df_filtered = df_new.filter(
    (F.col("report_date") >= 20140301) & (F.col("report_date") <= 20140831)
)
...
train_data, test_data = df_features.randomSplit([0.8, 0.2], seed=4254)
...
for target_col in target_cols:
    lr = LinearRegression(featuresCol='features', labelCol=target_col, maxIter=80, regParam=0.1, tol=1e-5)
